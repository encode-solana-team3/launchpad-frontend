...
import { findLaunchPoolAccount } from "@/utils/account";
...
const useCompleteLaunchpad = (launch_pad_pda: string) => {
  ...
  return useMutation({
    ...
    mutationFn: async () => {
      ...
      if (!poolData.authority.equals(wallet.publicKey)) {
        return Promise.reject(new Error("You are not the authority"));
      }
    ...
    onSuccess: ({ tx }) => {
      toast.update(toastRef.current!, {
        render: (
          <TxSubmitted
            message="Launch pool completed successfully"
            txHash={tx}
          />
        ),
        type: "success",
